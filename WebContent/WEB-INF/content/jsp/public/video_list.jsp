		...
		<div class="col-sm-10">
			...
			<div>
				...
				<s:iterator value="categories" var="c">
					<li class="nav-item"><a class="nav-link" href="category?id=${id }&page=${page}">${c.name}</a></li>
				</s:iterator>
				</ul>
			</div>
					...
					<ul class="pagination justify-content-center">
						<s:if test='page >0'>
							...
							<li class="page-item"><a class="page-link"
								href="category?id=${id }&page=0">首页</a></li>
						</s:if>
						<s:if test='prevpage>=0'>
							<li class="page-item"><a class="page-link"
								href="category?id=${id }&page=${prevpage}">上一页</a></li>
						</s:if>
						<s:iterator value="pages" var="p">

							<li class="page-item"><a class="page-link"
								href="category?id=${id }&page=${p}">${p+1}</a></li>
						</s:iterator>
						<s:if test='nextpage >0&&nextpage<maxPage'>

							<li class="page-item"><a class="page-link"
								href="category?id=${id }&page=${nextpage}">下一页</a></li>
						</s:if>
						<s:if test='maxPage >10'>
							<li class="page-item"><a class="page-link"
								href="category?id=${id }&page=${maxPage }">总共${maxPage+1 }页</a></li>
						</s:if>
					</ul>
				</nav>
